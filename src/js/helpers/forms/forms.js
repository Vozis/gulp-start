...
import { modulesList } from "../../files/modulesList.js";
...
export let formValidate = {
  ...
  formClean(form) {
    ...
    setTimeout(() => {
      ...
      if (modulesList.select) {
        let selects = form.querySelectorAll(".select");
        if (selects.length) {
          for (let index = 0; index < selects.length; index++) {
            const select = selects[index].querySelector("select");
            modulesList.select.selectBuild(select);
          }
        }
      }
    }, 0);
...
export function formSubmit(options = { validate: true }) {
  ...
  function formSent(form, responseResult = ``) {
    ...
    setTimeout(() => {
      if (modulesList.popup) {
        const popup = form.dataset.popupMessage;
        popup ? modulesList.popup.open(popup) : null;
      }
    }, 0);
    // Очищаем форму
    formValidate.formClean(form);
